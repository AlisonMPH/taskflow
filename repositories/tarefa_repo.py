...
class TarefaRepo:
    ...
    def inserir(cls, tarefa: Tarefa) -> Optional[Tarefa]:
        try:
            with obter_conexao() as conexao:
                cursor = conexao.cursor()
                cursor.execute(SQL_INSERIR_TAREFA, (
                    tarefa.titulo,
                    tarefa.descricao,
    ...
    @classmethod
    def obter_todos(cls) -> List[Tarefa]:
        try:
            with obter_conexao() as conexao:
                cursor = conexao.cursor()
                tuplas = cursor.execute(SQL_OBTER_TAREFAS_CLIENTE).fetchall()
    ...
    def alterar(cls, tarefa: Tarefa) -> bool:
        try:
            with obter_conexao() as conexao:
                cursor = conexao.cursor()
                cursor.execute(SQL_ATUALIZAR_TAREFA, (
                    tarefa.titulo,
                    tarefa.descricao,
                    tarefa.data_vencimento,
                    tarefa.id_categoria,
    ...
    @classmethod
    def obter_um(cls, id: int) -> Optional[Tarefa]:
        try:
            with obter_conexao() as conexao:
                cursor = conexao.cursor()
